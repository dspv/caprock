...
- **All data stays on the machine** — the daemon binds `127.0.0.1` only, and nothing about the user ever leaves. Three calls can leave the machine and each is one the user switched on: the release check asks GitHub for a version tag (`internal/update`), Gemini answers a question on the user's own key (`internal/gemini`), and the weekly report reaches the user's own Telegram bot (`internal/weekly`). None of them go to Caprock, and none carry anything about the user that the user did not send deliberately. Enforced by review of any new outbound HTTP client ([06-engineering-rules.md](06-engineering-rules.md)).
