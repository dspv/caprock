...
- **DeepSeek Harness transcripts** (`~/.dsh/sessions/<cwd>/<id>/session.v3.jsonl.zstd`). One append-only JSONL transcript per session, Zstandard-compressed, carrying the model, per-turn token usage (fresh input / cache read / output, reported separately), tool calls and prompts. Read by `internal/deepseek` the same way Codex's importer reads its rollouts — a poll that re-reads only changed files, keyed by the record's monotonic `seq`. Latency: poll cadence (5s), sessions are history rather than live. Cache *writes* stay zero — DSH reports reads and no write counterpart ([20-deepseek.md](20-deepseek.md)).
...
Append-only `events` table + materialized rollups (`session_stats`, `agent_stats`, `daily_stats`). DDL in [03-contracts.md § SQLite schema](03-contracts.md#sqlite-schema-ddl-v1). Note the DDL v1 `events` row carries `source` (`hook` | `transcript`) in addition to the fields above, because the same session is seen through both planes; the planes are reconciled by shared dedupe keys ([03-contracts.md § Hook shim](03-contracts.md#hook-shim)). Three extra kinds exist in code: `context.compact` (PreCompact hooks), and `context.clear` / `session.continue` for the `SessionEnd` reasons that leave the session running. `SessionStart` maps to `agent.spawn`; `Stop`/`SubagentStop` to `agent.stop` (subagents carry `agent_id`). **`SessionEnd` maps to `session.end` only when its reason is an exit** — `clear` becomes `context.clear` and `prompt_input_exit`/`other` become `session.continue`, because rollup retires a session on `session.end` and those reasons do not end one ([03-contracts.md § Hook shim](03-contracts.md#hook-shim)). Gemini answers are recorded as an ordinary `turn.user` + `turn.assistant` pair with `source = gemini`, so they are priced by the same table, searched by the same index and filtered by the same `agent` column as everything else ([ADR-023](08-decisions.md)).
