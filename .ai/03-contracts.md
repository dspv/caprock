...
The line is fitted to the terminal (`$COLUMNS`, else 80 — Claude Code pipes our stdout, so the file descriptor describes a pipe, not the window). Segments carry a rank and are dropped in order: token totals first, then the cache figure, then turns/steps. The plan windows are never dropped; they are the number that decides whether work can continue at all. The counters are ranked separately rather than as a block because the full line is 81 columns on a real session — with one rank, a standard 80-column terminal lost all four counters to save a single column.
