...
The line is fitted to the terminal (`$COLUMNS`, else 80 — Claude Code pipes our stdout, so the file descriptor describes a pipe, not the window). The line leads with the mark — the amber ⛰ the favicon draws — so it is identifiably Caprock's rather than an anonymous row of figures. Segments carry a rank and are dropped in order: the wordmark shrinks to the bare mark first (ten columns, and it carries nothing the mark does not — on an 80-column terminal that is the difference between showing turns/steps and showing none), then token totals, then the cache figure, then turns/steps. The mark itself is never dropped. The plan windows are never dropped; they are the number that decides whether work can continue at all. The counters are ranked separately rather than as a block because the full line is 81 columns on a real session — with one rank, a standard 80-column terminal lost all four counters to save a single column.
