...
-- migration 0013: the carry-forward scan's order, covering every column it reads
...
- **Why the previous rule was replaced.** Strict attribution charged a directory only when **every** file a turn touched was in it, and everything else — including every turn that touched no file — became "repository-wide work". On the owner's database that bucket was **87.6%** of `amarketer`'s spend: the user asked what a service cost and seven eighths of the answer was "we could not tell". Carry-forward gives the same repository `/app` 61.0% ($2090.67), `/.ai` 6.8%, `/app/tests` 2.8% (measured 2026-08-22, all time).
