...
`caprock service install|uninstall|status` registers the daemon with the OS's own login-time supervisor, so it survives a reboot. It writes exactly one file, always inside the user's own home/config locations and never in `~/.claude/`; `internal/service` generates and inspects it as a pure function of a `Plan` (executable path, data dir, port, hive dir), and shells out to the platform tool only to load/unload.

**The definition file per OS:**

- **macOS** — `~/Library/LaunchAgents/dev.caprock.daemon.plist`, registered with `launchctl bootstrap gui/<uid>` (bootout first, so a reinstall replaces rather than duplicates). `RunAtLoad`, `KeepAlive.SuccessfulExit=false`, `ProcessType=Background`, `LowPriorityIO`, stdout/stderr to `<data_dir>/service.log`.
- **Linux** — `~/.config/systemd/user/caprock.service` (honouring `XDG_CONFIG_HOME`), enabled with `systemctl --user enable --now`. `Type=simple`, `Restart=on-failure`, `WantedBy=default.target`; `StartLimitIntervalSec`/`StartLimitBurst` in `[Unit]`, not `[Service]` — systemd v229+ ignores them under `[Service]`. No systemd user session → an actionable error and **nothing written**.
- **Windows** — `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\caprock.cmd`, CRLF, `start "" /b`. The Startup folder over a Scheduled Task because it is a plain user-owned file: generated, inspected and removed with ordinary file I/O, so the generation logic is unit-tested on all three OSes and installing it mutates no machine-wide store. The trade-off is stated in the README: no per-user crash supervisor exists without admin rights, so Windows restarts at logon but not mid-session.

**What the service runs:** `<abs path to caprock> up --foreground --no-open --no-hooks [--port N] [--hive DIR]`, with `CAPROCK_DATA_DIR` set to the resolved data dir. `--foreground` is mandatory on every platform — the supervisor tracks the process it started, so the detach `caprock up` normally performs would read as an instant exit and restart forever. `--no-hooks` keeps hook/statusline registration an interactive consent decision ([§ Statusline](#statusline)), never something a login agent does. The absolute path comes from `os.Executable()`: a login agent does not inherit the shell's `PATH`.

**Idempotency and state:** install rewrites the file and re-registers, so running it twice yields one file with identical contents; a file that differs from what the current binary would write reads as "installed, stale" and install refreshes it. Uninstall deregisters, then removes the file; neither step being present is a clean no-op, not an error. `status` reports the file state and the supervisor's own registration separately, because they can legitimately disagree (a hand-deleted plist stays loaded until logout).
