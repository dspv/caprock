...
- **Strict attribution.** A turn's cost is charged to a directory only when **every** path it touched is in that one directory; the figure is then exact rather than modelled. A turn spanning several directories is **not** split, pro-rated, or assigned to the largest share — it goes whole into one labelled row, so the parts still sum to the repository total and no number on screen is an estimate (rule 6).
- **The touch rule.** A tool that names a **file** touches that file's directory: `Read`, `Edit`, `Write`, `NotebookEdit`. `Bash` does not, even when its command contains a path — `grep -r foo /services` reads a tree and `cd /services/api && go build ./...` names one directory while touching many, so parsing intent out of a command line would be inference presented as measurement. `Bash` is the largest tool by volume (32581 of 65915 calls on the owner's database), which is why a large share lands unattributed and the UI says so rather than redistributing it.
- **Outside the repository.** A turn touching `/tmp` or another checkout is not charged to this repository's directories — the directory is not one of them — but still counts toward the repository total, so it lands unattributed. The money was really spent there; only its location within the tree is unknown.
- **Columns, not `json_extract` on read.** `/v1/stats/summary` is polled. Measured through the Go driver on the owner's 191k-event database (2026-08-22, 30d): the summary answers in **~152 ms** warm, while `json_extract` over the 48212 `tool.pre` rows in that range costs **~215 ms** on its own. Resolved at ingest the same scan is **~9 ms**. The summary now answers in **~250 ms**, the added cost being the per-turn scan attribution needs (~89 ms), not the touch lookup.
- **Backfill** — `touch_dir` is filled by `Store.backfillTouch` (Go, not SQL: the dirname must be cut with the same normalization the ingest path uses, and SQLite has no `reverse`), gated by the `touch_backfilled` meta key because a pathless tool call keeps `touch_dir` NULL forever. `msg_id` on historical `tool.pre` rows is **not** in the database at all — the payload never carried it — so `ingest.BackfillToolMessageIDs` reads it back once from the transcripts still on disk, gated by `tool_link_backfilled`. Rows whose transcript is gone stay unlinked and report as unattributed: degraded, never wrong.
