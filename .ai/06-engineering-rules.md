...
- **No outbound network calls from the daemon or shim.** The only sockets are `127.0.0.1` listeners and loopback POSTs from the shim. CI greps for `net/http` client use outside `cmd/caprock-hook` (the shim's loopback POST) and the localhost API client in `ui/`. Local-first is a promise, not a default.
