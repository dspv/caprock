...
- Every third-party action in these workflows is pinned to a full commit SHA, with the human-readable release retained in a comment. Dependabot (`.github/dependabot.yml`) checks GitHub Actions, the root Go module, and `ui/` npm dependencies weekly and opens bounded update PRs.
