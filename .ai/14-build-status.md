...
- **Endpoints and dispatch with no HTTP/CLI test:** the whole Phase 2 API (`/v1/tasks/{id}/verify`, `/v1/orchestrator/start`, approve/**reject**, the 409 error path, the 501-when-disabled path), `GET /v1/history`, the `/term` terminal WebSocket (snapshot-on-connect + streaming), auto-pause (owned-only + opt-in, extracted to `Daemon.maybeAutoPause`), the `cmd/caprock` CLI dispatch (all subcommands incl. the hidden `hook` fallback, `shimCommand`), and a pin on the exact 8 hook events. Net: ~19 new tests (109 test functions total), all green on the 3-OS matrix.
