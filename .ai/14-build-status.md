...
"Automatic approval review model". Treated as user work it raised the
...
every user-work total (turns, tokens, cost, model mix, projects roll-up, the
daily cap, the weekly report) and reported beside them as a quiet "background
usage" line with no dollar value. Raw events stay for auditability; only the
aggregates exclude them.

**The first pass got the granularity wrong, and running the daemon on the real
machine proved it.** The review turns were written into the *same* session as
the `gpt-5.6-sol` turns being reviewed — Codex reuses the reviewed session's id
— so the first session-level flag hid 8.1 million tokens of real work to get rid
of 190k tokens of review. Classification is therefore **per-event**
(`events.internal`, migration 0024) via `internal/modelclass` (explicit
allow-list, not a prefix match), and the session's real model is preserved so a
review turn cannot overwrite it. A future id must be investigated before
Caprock hides it. Pinned by sabotage: `TestCodexAutoReviewIsBackgroundUsage`
(the mixed session keeps its real work), `TestInternalEventMigrationBackfillsAndRemovesUserRollups`,
and the UI `Unpriced.test.tsx`.
...
**A session ended twelve hours after it ended, and the live pulse showed the consequence.** The Now screen counted a whole day's finished sessions as live — "14 sessions, 0 active" — and the pulse drew a row per known session, so a day in one repository was six identical `caprock` rows over six flat hairlines. `rollup`'s own comment said `ended` came from "SessionEnd/kill", but **nothing had ever registered that hook**: the only path to `ended` was the 12-hour staleness sweep. The comment described an intention, and for months nobody read it against `internal/hooks.Events`. The shim now registers `SessionEnd`, and the sweep drops to an hour as the backstop it should always have been — the case where the hook never fired (`kill -9`, a closed terminal, a dead host). Ending early is cheap because the session upsert revives an ended session on its next event, the same rule that already stopped a daemon restart from burying a working agent.

**The pulse now draws only what it can draw**, and its cost column answers the question the bars ask. Ended sessions and tracks with no events in the window are dropped — an hour of flat hairline says only that a session exists, and six of them read as a broken chart rather than as silence. The row's figure was the session's *lifetime* cost sitting beside an hour of bars: `$4,053.39` next to a `$101.85` day, two true numbers answering different questions. It now sums the window, with the lifetime on hover. A pinned test on the hook list caught the intended change and had to be updated deliberately, which is exactly what that pin is for.

**Caprock was killing the work it exists to watch.** Restarting the daemon SIGKILLed every session it had spawned — so *upgrading Caprock* destroyed whatever the user had running under it, mid-turn, with nothing flushed and no warning. It surfaced by accident: a `make reload` during this session killed the user's live agent in another repository, and the daemon log timestamped the two events to the same tenth of a second. The fix is the obvious one and was never written: ask (`SIGTERM`), wait five seconds so Claude Code can write out its transcript, kill only what refuses. The grace period is spent once for all sessions rather than once per session, and a paused session is resumed first — it cannot act on a signal otherwise, so it would sit out the whole window and be killed anyway. The upgrade banner now names how many sessions will close, and only when there are any; sessions the user started themselves were never affected. Both tests fail if the polite request is swapped back for the kill.
