...
release-ready: check ## Everything `check` does, plus the things only a release forgets
	@echo ""
	@echo "→ the site's changelog"
	@# The site keeps its own copy of CHANGELOG.md, regenerated by a script in
	@# that repository. Nothing enforces running it, so a release cycle that
	@# skips the step leaves caprock.dev showing a version that no longer
	@# exists — which is what happened on 0.49.1 through 0.49.3, and was only
	@# noticed because a stray background job was still watching for a string.
	@#
	@# A warning rather than a failure: the site is a sibling checkout that not
	@# everyone building this repo has, and a gate that cannot run everywhere is
	@# a gate people learn to skip.
	@if [ -d ../caprock-web ]; then \
		if ! (cd ../caprock-web && node scripts/sync-changelog.mjs --check >/dev/null 2>&1); then \
			echo "  ✗ caprock.dev is behind CHANGELOG.md."; \
			echo "    cd ../caprock-web && npm run sync:changelog && git commit -am 'docs(changelog): sync' && git push"; \
			exit 1; \
		fi; \
		echo "  ✓ the site's changelog matches"; \
	else \
		echo "  – no ../caprock-web checkout; sync it wherever you keep it"; \
	fi
	@echo ""
	@echo "Ready. Tag with: git tag -a vX.Y.Z -m '...' && git push origin vX.Y.Z"
