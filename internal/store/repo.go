...
// first path segment under the root, or "." for work at the root itself.
...
// repoLabel is the display name of a repository root: its basename, and for a
// root whose basename would be ambiguous or meaningless on its own (`src`,
// `repo`, a bare drive or `/`), enough of the parent to tell two of them apart.
...
// unrootedInfo labels a directory with no repository above it. It never invents
// a repository: Root stays empty, and the label is the path itself, shortened
// to the home-relative or last-two-segment form so that two different
// directories with the same basename stay two different rows.
func unrootedInfo(cwd string) RepoInfo {
	return RepoInfo{Root: "", Repo: shortPathLabel(cwd), Path: ""}
}
